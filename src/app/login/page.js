...
export default function LoginPage() {
  ...
  // States
  const [step, setStep] = useState("input"); // 'input' (for email/phone) or 'otp'
  ...
  // Helper to determine if we should send 'email' or 'phoneNumber' key to backend
  const getPayload = (value) => {
    const isEmail = value.includes("@");
    return isEmail ? { email: value.trim() } : { phoneNumber: value.trim() };
  };

  // 1. Handle sending the initial OTP
  const handleLoginSubmit = async (e) => {
    ...
    try {
      // Constructs payload as {email: ...} OR {phoneNumber: ...}
      const payload = getPayload(identifier);
  ...
  // 2. Handle Resending OTP
  const handleResend = async () => {
    setErrorMessage("");
    try {
      const payload = { ...getPayload(identifier), type: "LOGIN" };
  ...
  const handleVerifyOtp = async (e) => {
    ...
    if (finalOtp.length === 4) {
      try {
        // Construct payload with dynamic key + otp + type
        const payload = {
            ...
            <form onSubmit={handleVerifyOtp} className="p-8 space-y-8">
              ...
              <p className="text-center text-xs text-slate-400">
                ...
                <button
                  ...
                  onClick={handleResend}
                  disabled={isResending}
                  className="text-[#072369] font-bold hover:underline disabled:opacity-50"
                >
