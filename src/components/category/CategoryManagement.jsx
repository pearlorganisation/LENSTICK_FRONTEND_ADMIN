...
const CategoryManagement = () => {
  ...
  useEffect(() => {
    if (!editId) {
      // Only auto-gen slug for new categories
  ...
  const categoryTree = useMemo(() => {
    ...
    const buildTree = (items, parentId = null) => {
      return items
        .filter(
          (item) =>
            (item.parentCategory?._id || item.parentCategory) ===
            (parentId || null)
        )
  ...
  const paginatedTree = categoryTree.slice(
  ...
  const handleDeleteClick = async (id) => {
    if (
      window.confirm("Are you sure? This will also delete the image from S3.")
    ) {
      try {
